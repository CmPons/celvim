...
local function clear_lsp_log()
  local path = vim.fs.normalize(vim.fn.stdpath("data") .. "/lsp.log")

  info("Clearing lsp.log on path: ", path)

  local succeeded, errmsg = os.remove(path)
  if succeeded == false then
    error("Failed to clear lsp.log file " .. errmsg)
  end
